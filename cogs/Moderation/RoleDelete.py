...
class DeleteRole(commands.Cog):
    ...
    async def delete_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if role.position >= interaction.guild.me.top_role.position:
            await interaction.response.send_message("I cannot delete this role as it is higher than my highest role!", ephemeral=True)
            return

        try:
            await role.delete()
            await interaction.response.send_message(f"Role {role.name} deleted!", ephemeral=False)
        except discord.Forbidden:
            await interaction.response.send_message("I do not have permission to delete this role!", ephemeral=True)
        except discord.HTTPException:
            await interaction.response.send_message("Failed to delete the role due to an HTTP error!", ephemeral=True)
